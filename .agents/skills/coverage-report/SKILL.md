...
allowed-tools: Bash(npx vitest*), Bash(mkdir*), Write
---
...
5. **Ensure the output directory exists** by running:
   ```bash
   mkdir -p test_coverage_reports
   ```

6. **Generate `test_coverage_reports/Sprint-$SPRINT-Code-Coverage-Report.md`** using this exact template format:
...
7. **Show the user** the generated file path and a summary of the key metrics.
...
- For the coverage trend (section 4), look for any existing `Sprint-*-Code-Coverage-Report.md` files in `test_coverage_reports/` and compare against the most recent one.
