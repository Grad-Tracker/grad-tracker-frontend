...
export default function PlannerPage() {
  ...
  const [removePrereqDialog, setRemovePrereqDialog] = useState<{
    open: boolean;
    course: Course | null;
    termId: number | null;
    dependents: number[];
    source: "drag" | "drawer";
  }>({ open: false, course: null, termId: null, dependents: [], source: "drag" });
    ...
    async function init() {
      ...
      try {
        ...
        if (!selectedBreadthId) {
          const legacyBreadth = getLegacyBreadthPackageFromStorage(sid);
          if (legacyBreadth) {
            await updateBreadthPackageId(sid, legacyBreadth);
            clearLegacyBreadthPackageStorage(sid);
  ...
  const handleRemoveCourseFromTerm = useCallback(
    ...
    [studentId, activePlanId, removingCourseFromDrawer, prereqEdges, courseTermIndex, completedIds]
  );

  // ── Confirm remove with dependents ─────────────────────
  const handleConfirmRemoveWithDependents = useCallback(async () => {
    const { course, termId, source } = removePrereqDialog;
    if (!course || !termId || !studentId || !activePlanId) return;

    if (source === "drawer") setRemovingCourseFromDrawer(true);
    try {
      await removePlannedCourse(
        studentId,
        termId,
      ...
      if (source === "drawer") setRemovingCourseFromDrawer(false);
    }
  }, [removePrereqDialog, studentId, activePlanId]);

  // ── Remove-with-dependents check ────────────────────────
  function checkAndRemoveCourse(
    course: Course,
    termId: number,
    source: "drag" | "drawer",
  ) {
    ...
    if (broken.length > 0) {
      ...
      return false; // blocked — dialog will handle
    }
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (fromTermId && (overId === "course-panel" || !overId.startsWith("term-"))) {
      // Check dependents first
      if (!checkAndRemoveCourse(course, fromTermId, "drag")) return;
  ...
  return (
    ...
    >
      ...
      <ConfirmationDialog
        open={removePrereqDialog.open}
        onOpenChange={(open) =>
          setRemovePrereqDialog((prev) => ({ ...prev, open }))
        }
        onConfirm={handleConfirmRemoveWithDependents}
        title={`Remove ${removePrereqDialog.course ? getCourseActivityLabel(removePrereqDialog.course) : "course"}?`}
        confirmText="Remove anyway"
        confirmColor="red"
      >
        <Text>
          Removing this course will leave the following courses without a
          prerequisite:
        </Text>
        <Box as="ul" mt="2" pl="5">
          {removePrereqDialog.dependents.map((id) => {
            const c = courseById.get(id);
            return (
              <li key={id}>
                {c ? `${c.subject} ${c.number} — ${c.title}` : `Course #${id}`}
              </li>
            );
          })}
        </Box>
